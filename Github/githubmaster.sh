...
do
  if [[ "$i" =~ "GCMC" ]]; then
    ...
    root=`pwd`
	  cd $i
	  sed -i "s#RUN_DIR#$(pwd)#g" serial.inp
	  sed -i "s#COMMAND#./GOMC_CPU_GCMC +p4 in.conf \&> out.log#g" serial.inp
	  cd $root
  elif [[ "$i" =~ "GEMC" ]]; then
    cp ./GOMC/bin/GOMC_CPU_GEMC "$i"
    cp ./job_scripts/openmp.inp "$i"
    root=`pwd`
	  cd $i
	  sed -i "s#RUN_DIR#$(pwd)#g" serial.inp
	  sed -i "s#COMMAND#./GOMC_CPU_GEMC +p4 in.conf \&> out.log#g" serial.inp
	  cd $root
  elif [[ "$i" =~ "NVT" ]]; then
    cp ./GOMC/bin/GOMC_CPU_NVT "$i"
    cp ./job_scripts/openmp.inp "$i"
    root=`pwd`
	  cd $i
	  sed -i "s#RUN_DIR#$(pwd)#g" serial.inp
	  sed -i "s#COMMAND#./GOMC_CPU_NVT +p4 in.conf \&> out.log#g" serial.inp
	  cd $root
  elif [[ "$i" =~ "NPT" ]]; then
    cp ./GOMC/bin/GOMC_CPU_NPT "$i"
    cp ./job_scripts/openmp.inp "$i"
    root=`pwd`
	  cd $i
	  sed -i "s#RUN_DIR#$(pwd)#g" serial.inp
	  sed -i "s#COMMAND#./GOMC_CPU_NPT +p4 in.conf \&> out.log#g" serial.inp
	  cd $root
  else
    echo -e "[ERROR] Couldn't detect the ensemble type."
    echo -e "\tExample: /input/GCMC/case01/"
